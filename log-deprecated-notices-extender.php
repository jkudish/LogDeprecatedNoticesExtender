...
if ( ! class_exists( 'Deprecated_Log_Extender' ) ) {
	...
	class Deprecated_Log_Extender {
		...
		public function count() {
			...
			$this->count = $wpdb->get_var( $wpdb->prepare( 
				"SELECT COUNT(ID) 
				 FROM $wpdb->posts
				 WHERE post_type = %s 
				 AND post_date > %s 
				 AND post_status = %s",
				 Deprecated_Log::pt,
				 Deprecated_Log::$instance->options[ 'last_viewed' ],
				 'publish' ) );
			$all_deprecated_notes = wp_count_posts( Deprecated_Log::pt );
			
			$this->count = number_format_i18n( $this->count ) . '/'  . number_format_i18n( $all_deprecated_notes->publish );
			
			$this->title = ( $this->count ) ? sprintf( __( 'Deprecated Calls <small>%s</small>', 'log-deprecated' ), $this->count ) : __( 'Deprecated Calls', 'log-deprecated' );
		}

		/**
		 * hooks into the toolbar to add our new menu item
		 * 
		 * @param  $toolbar the wp admin bar object
		 * @return void
		 */
